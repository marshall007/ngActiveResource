  ...
  this.$get = [
    ...
    function (Helpers, Mime) {
      function API(klass, pk) {
        ...
        this.deleteURL = '';
        this.updateURL = '';
        this.set = function (url, override) {
          if (url.slice(-1) != '/')
            url = url + '/';
          if (override) {
    ...
    function (json, mixin, Associations, Helpers, deferred) {
      ...
      function setAssociationsAndUpdate(instance, response, options) {
        if (options && options.update == false)
          options.update = true;
        var associations = Associations.get(instance);
        return associations.belongsTo.reduce(function (promise, foreignRel) {
          return promise.then(function (res) {
            if (!res[foreignRel.foreignKey])
              return res;
            var association = foreignRel.klass;
            var associatedName = association.name.camelize();
            var foreignKey = foreignRel.foreignKey;
            var query = res[foreignKey];
            ...
            return foreignRel.klass.find(query, queryOptions).then(function (association) {
              res[associatedName] = association;
              delete res[foreignKey];
              return res;
            });
          });
        }, deferred(response)).then(function (res) {
          return updateLocalInstance(instance, res, options);
        });
      }
      ;
      function eagerLoad(instance) {
        var associations = Associations.get(instance);
        var dependentList = associations.hasMany.concat(associations.hasOne);
        return dependentList.reduce(function (promise, association) {
          return promise.then(function (instance) {
            ...
            return association.klass.where(query, { lazy: true }).then(function (response) {
